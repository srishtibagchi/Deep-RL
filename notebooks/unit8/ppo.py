...
def make_env(env_id, seed, idx, capture_video, run_name):
  def thunk():
    ...
    if capture_video:
      if idx == 0:
        env = gym.wrappers.RecordVideo(env, f"videos/{run_name}", record_video_trigger=lambda t: t % 100 == 0)
    env.seed(seed)
...
def parse_args():
  ...
  parser.add_argument('--track', type=lambda x:bool(strtobool(x)), default=True,
                    nargs='?', const=True, help='if toggled, experiment will be tracked with wandb')
  parser.add_argument('--wandb-project-name', type=str, default="cleanRL",
                    help='wandb project name')
  parser.add_argument('--wandb-entity', type=str, default=None,
                    help='wandb entity (team) of the project')
  parser.add_argument('--capture-video', type=lambda x:bool(strtobool(x)), default=False,
                    nargs='?', const=True, help='whether to capture videos of the agent performances in videos folder')
  ...
  parser.add_argument('--norm-adv', type=lambda x:bool(strtobool(x)), default=True,
                      help='normalize the advantages')
  parser.add_argument('--clip-coef', type=float, default=0.2,
                      help='the surrogate clipping coefficient')
  parser.add_argument('--clip-vloss', type=lambda x:bool(strtobool(x)), default=True,
                      help='toggles whether to use a clipped loss for the value function, as per the paper')
  ...
  parser.add_argument("--repo-id", type=str, default="srishtibagchi/ppo-CartPole-v1", help="id of the model repository from the Hugging Face Hub {username/repo_name}")
    ...
    for step in range(0, args.num_steps):
      ...
      for epoch in range(args.update_epochs):
        ...
        for start in range(0, args.batch_size, args.minibatch_size):
          ...
          optimizer.zero_grad()
          loss.backward()
          nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)  # global gradient clipping, if enabled
          optimizer.step()

        # early stopping: stop the epochs if approximate kl divergence goes over a preset threshold
      ...
      var_y = np.var(y_true)
      # explained variance: indicates if value function is good indicator of the returns
